...
if "extracted_data" not in st.session_state:
    st.session_state.extracted_data = {page: pd.DataFrame(columns=["Sender", "Message", "Phone", "Created"]) 
...
for i, (page_name, token) in enumerate(PAGES.items()):
    with tabs[i]:
        ...
        if st.button(f"Extract from {page_name}"):
            ...
            st.session_state.extracted_data[page_name] = pd.concat(
                [st.session_state.extracted_data[page_name], new_df],
                ignore_index=True
            )

            st.success(f"Found {len(new_df)} new phone numbers. Total: {len(st.session_state.extracted_data[page_name])}")

        # Display the current DataFrame (persistent across runs)
        st.dataframe(st.session_state.extracted_data[page_name])
